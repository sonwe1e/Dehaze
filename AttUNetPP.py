...
class UNet_Nested(nn.Module):
    ...
    def __init__(self, in_channels=3, n_classes=3, feature_scale=2, is_deconv=True, is_ReLU=False, is_ds=True):
        ...
        # Skfusion
        self.SkFusion3 = SKFusion(filters[-2], height=2, reduction=8)
        self.SkFusion2 = SKFusion(filters[-3], height=2, reduction=8)
        self.SkFusion1 = SKFusion(filters[-4], height=2, reduction=8)
        ...
        self.conv30 = unetConv2(filters[2], filters[3], self.is_ReLU, n=4)
        ...
        self.final_4 = nn.Conv2d(filters[0], n_classes, 3, 1, 1)

        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Linear(filters[0], 1)
    ...
    def forward(self, inputs):
        ...
        X_21 = self.SkFusion3([X_21, X_20])
        # column : 2
        X_02 = self.up_concat02(X_11, X_00, X_01)
        X_12 = self.up_concat12(X_21, X_10, X_11)
        X_12 = self.SkFusion2([X_12, X_10])
        # column : 3
        X_03 = self.up_concat03(X_12, X_00, X_01, X_02)
        X_03 = self.SkFusion1([X_03, X_02])

        # final layer
        final_4 = self.final_4(X_03)

        A = self.avg_pool(X_03)
        A = A.view(A.size(0), -1)
        A = self.fc(A).unsqueeze(2).unsqueeze(3)

        result4 = final_4 * inputs - A * (1 - final_4)
